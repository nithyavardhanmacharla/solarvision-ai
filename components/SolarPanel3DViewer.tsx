...
export function SolarPanel3DViewer({
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      e.preventDefault();
      e.stopPropagation();
      cameraRadius = Math.max(3.5, Math.min(12.0, cameraRadius + e.deltaY * 0.005));
    };

    // --- BODY SCROLL LOCK ---
    // Freeze the entire document when touching the 3D viewer.
    // This is the only 100% reliable way to prevent mobile scroll during 3D rotation.
    let savedBodyStyles = { overflow: '', position: '', top: '', width: '', height: '' };
    let scrollY = 0;
    ...
    // Mouse: mousedown on canvas, mousemove/mouseup on window
    if (canvasElem) {
    ...
    // Touch & Wheel: on BOTH canvas and container with { passive: false }
    if (canvasElem) {
      ...
      canvasElem.addEventListener('touchend', handleTouchEnd, { passive: false });
      canvasElem.addEventListener('touchcancel', handleTouchEnd, { passive: false });
    }
    if (containerElem) {
      ...
      containerElem.addEventListener('touchend', handleTouchEnd, { passive: false });
      containerElem.addEventListener('touchcancel', handleTouchEnd, { passive: false });
    }
    ...
    const animate = () => {
      ...
      camera.position.x = cameraRadius * Math.sin(cameraRotY) * Math.cos(cameraRotX);
      camera.position.y = cameraRadius * Math.sin(cameraRotX);
      camera.position.z = cameraRadius * Math.cos(cameraRotY) * Math.cos(cameraRotX);
    ...
    return () => {
      ...
      if (canvasElem) {
        ...
        canvasElem.removeEventListener('touchcancel', handleTouchEnd);
      }
      ...
      if (containerElem) {
        ...
        containerElem.removeEventListener('touchcancel', handleTouchEnd);
      }
      // Ensure body is unlocked if component unmounts during drag
      if (isDragging) {
        unlockBodyScroll();
      }
      renderer.dispose();
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl bg-zinc-950 border border-zinc-800 touch-none select-none overscroll-contain ${className}`}
    ...
    >
      <div
        ref={containerRef}
        className="w-full h-[260px] sm:h-[320px] cursor-grab active:cursor-grabbing relative touch-none select-none overscroll-contain"
        style={{ touchAction: 'none' }}
        onTouchStart={(e) => e.stopPropagation()}
        onTouchMove={(e) => e.stopPropagation()}
        onWheel={(e) => e.stopPropagation()}
      >
        <canvas
          ref={canvasRef}
          className="w-full h-full block touch-none select-none overscroll-contain"
          style={{ touchAction: 'none' }}
          onTouchStart={(e) => e.stopPropagation()}
          onTouchMove={(e) => e.stopPropagation()}
          onWheel={(e) => e.stopPropagation()}
