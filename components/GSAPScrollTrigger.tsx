  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      ScrollTrigger.batch('[data-gsap="fade-up"]', {
        ...
        onLeaveBack: (batch) => {
          gsap.to(batch, { opacity: 0, y: 30, duration: 0.4, overwrite: 'auto' });
        }
      });
      ...
      ScrollTrigger.batch('[data-gsap="slide-left"]', {
        onEnter: (batch) => {
          gsap.fromTo(
            batch,
            { opacity: 0, x: -50 },
            { opacity: 1, x: 0, duration: 0.9, ease: 'power3.out', stagger: 0.12, overwrite: 'auto' }
          );
        }
      });
