...
export function MapExplorer({
  ...
  }, [isMapModalOpen, location.lat, location.lng, onSelectLocation, scrollWheelZoomEnabled, tileMode]);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      const res = await fetch(`/api/solar/geocode?q=${encodeURIComponent(query)}`);
      if (res.ok) {
